...
-- If successful, return JSON with 
 ...
 ----"RESULT" key and the value is a list of the sensors in JSON objects. Each object has "id", "sensor name" and "type" keys and values from the db
-- If failed, return JSON with "MESSAGE" key and value f"Error retrieving sensors {error}" (error from the execution of the command) - code 400
...
def get_sensors():
    try:
        ...
        if not type: #invalid/missing message
            ...
            sql_command = "SELECT * FROM SENSORS"
            if type == "all":
                result = db_obj.execute_sql(sql_command)
            else:
                result = db_obj.execute_sql(f"{sql_command} WHERE type='{type}'")
            if result[0]:
...
Parameters:
  No parameters 
Returns:
-- If failed in retrieving the locations, return JSON object with "MESSAGE" key and value f"Error retrieving locations {error}" (error from the execution of the command) - code 400
-- If successful, return JSON object with 
 ----"MESSAGE" key and value "Succeeded: data retrieved successfully"
 ----"RESULT" key and the value is a list of the locarions in JSON objects. Each object has "location" and "location_id"
-- If an exception occured in the whole function, return JSON object with "MESSAGE" key and value f"Exception {e}" where e is the exception message - code 500
- Example of the return:
{"MESSAGE": "Succeeded: data retrieved successfully", "RESULT":[{"location":"Main Gate","location_id":1},{"location":"Left Lower Corner","location_id":2},{"location":"Right Lower Corner","location_id":3},{"location":"Left Upper Corner","location_id":4},{"location":"Right Upper Corner","location_id":5},{"location":"Middle","location_id":6}]}
NOTE: location and location_id order is not relevent as these elements are accessed by KEY and not index (check dictionaries in python for more clarification)
'''
...
Returns:
-- If any of the IDs is not a valid integer, return JSON with key "MESSAGE" and value "Error: IDs are not a valid integer" - code 400
-- If failed in retrieving the data, return JSON object with "MESSAGE" key and value f"Error retrieving sensors in location {error}" (error from the execution of the command) - code 400
-- If succeeded, return JSON object with :
----"MESSAGE" key and value "Succeeded: data retrieved successfully" 
----"RESULT" key and the value is a list of the sensors in location (details below) in JSON objects. Each object has "location_id", "sensor_id" and "sensor_location_id"
...
Returns:
-- If either of the dates is not a valid date, return JSON object with key "MESSAGE" and value "Error: Dates are invalid or wrong format" - code 400
-- If succeeded, return JSON object with :
----"MESSAGE" key and value "Succeeded: data retrieved successfully" 
----"RESULT" key and the value is a list of logs(details below) in JSON objects. Each object has log_id, sensor_location id, timestamp and value keys
...
Return: 
-- If any parameter is missing, return JSON with "MESSAGE" key and value "Failed: name and/or type parameters are missing" - code 400
-- If failed to add sensor, return in JSON object "MESSAGE" key with value f"Failed to add sensor {error}" (error from the execution of the command) - code 400
...
Returns:
-- if sensor or location id is not available in parameters, return in in JSON "MESSAGE" key with value "Failed: sensor id and/or location id parameters are missing" - code 400
-- if sensor or location id is not an integer, return in JSON "MESSAGE" key with value "Error: IDs are not a valid integer" - code 400
-- if insertion succeeded, return in in JSON "MESSAGE" key "Succeeded: Sensor added in location successfully" and code 200.
-- Exception: return json with key "MESSAGE" and value f"Exception {e}" where e is the exception error. Code 500
'''
...
-- If the ID is not a valid integer, return JSON object with key "MESSAGE" and value "Error: ID is not a valid integer" - code 400
-- If the timestamp is not a valid date or valid format, return JSON oject with key "MESSAGE" and value "Error: Dates are invalid or wrong format" - code 400
...
-- If failed then the MESSAGE value is "Failed to add log entry {error}" and error is any error returned from the transaction. code 400
-- Exception: return json with key "MESSAGE" and value f"Exception {e}" where e is the exception error. Code 500
'''
...
Returns:
-- if sensor parameter is missing, return JSON object with key "MESSAGE" and value "Failed: sensor parameter is missing" - code 400
-- If successful: return JSON object with key "MESSAGE" and value "Succeeded: Sensor deleted successfully" - code 200
-- If failure: return JSON object with key "MESSAGE" and value "Failed to delete sensor {error}" where error is any error message from the execution. - code 400
-- Exception: return JSON object with key "MESSAGE" and value f"Exception {e}" where e is the exception error. Code 500
'''
...
Return:
-- If sensor id or location id is missing, return JSON object with key "MESSAGE" and value "Failed: sensor id and/or location id parameters are missing" - code 400
...
-- If deletion succeeded, return JSON object with key "MESSAGE" and value "Succeeded: Sensor in location is deleted successfully" - code 200
...
Returns:
-- If no log_id parameter, return JSON object with key "MESSAGE" and value "Failed: log_id parameter is missing" - code 400
-- If the ID is not a valid integer, return JSON object with key "MESSAGE" and value "Error: ID is not a valid integer" - code 400
-- If deletion succeeded, return JSON object with key "MESSAGE" and value "Succeeded: log entry is deleted successfully" - code 200
-- If failed: return JSON object with key "MESSAGE" and value "Failed to delete log entry {error}" where error is any error message from the execution. - code 400
-- Exception: return JSON object with key "MESSAGE" and value f"Exception {e}" where e is the exception error. Code 500
'''
